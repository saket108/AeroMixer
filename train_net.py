...
def train(
    ...
    checkpoint_period = (
        ...
        if "debug" not in output_dir
        else 1
    )
    val_after = (
        cfg.SOLVER.EVAL_AFTER * iter_per_epoch if "debug" not in output_dir else 0
    )
    val_period = (
        cfg.SOLVER.EVAL_PERIOD * iter_per_epoch if "debug" not in output_dir else 1
    )

    frozen_backbone_bn = (
        "vit" not in cfg.MODEL.BACKBONE.CONV_BODY.lower()
    ) and cfg.MODEL.BACKBONE.FROZEN_BN
...
def run_test(cfg, model, distributed):
    ...
    data_loaders_test, vocabularies_val, _ = make_data_loader(
...
def main():
    ...
    parser.add_argument(
        "--seed", type=int, default=2, help="Manual seed at the begining."
    )
    parser.add_argument(
        "--set_split", type=int, default=-1, help="Set the dataset split"
    )
    parser.add_argument(
        "--verbose-startup",
