...
def load_image_lists(cfg, is_train):
    ...
    """
    logger.info("Loading {} data list...".format("training" if is_train else "testing"))
    list_filenames = [
        ...
        with pathmgr.open(list_filename, "r") as f:
            ...
            for line in f:
                ...
                if 'debug' in cfg.OUTPUT_DIR:
                    if video_name not in debug_videos:
                        debug_videos.append(video_name)  # first 10 videos for debug
                    if len(debug_videos) == 10:
                        break
...
def load_boxes_and_labels(cfg, mode):
    ...
    if 'openworld' in cfg.AVA.ANNOTATION_DIR:
        gt_lists = [os.path.join(cfg.AVA.CW_SPLIT_DIR, gt_file) for gt_file in gt_lists]
    ann_filenames = [
...
def parse_bboxes_file(
    ann_filenames, ann_is_gt_box, detect_thresh=0.9, boxes_sample_rate=1, debug=False
):
    """
