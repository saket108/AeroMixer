...
def get_parameter_groups(model,
    ...
    param_names = [name for g, pdata in parameter_group_names.items() for name in pdata['params']]
    return list(parameter_group_vars.values()), param_names
...
def make_optimizer(cfg, model):
    ...
    if 'vit' in backbone_arch or 'vip' in backbone_arch:
        ...
        backbone_parameters = []
        backbone_parameters, param_names = get_parameter_groups(model.backbone,
                                        weight_decay,
        ...
        for name, p in model.named_parameters():
            if ("backbone" not in name) and p.requires_grad:
                rest_parameters['params'].append(p)  # stm_head and lateral_convs params
...
def make_lr_scheduler(cfg, optimizer, iter_per_epoch):
