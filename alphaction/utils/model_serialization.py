...
def exclude_layers(model_state_dict, excluded):
    model_state_dict_new = OrderedDict()
    for key, value in model_state_dict.items():
        if any([exc in key for exc in excluded]):
            continue
        model_state_dict_new[key] = value
    return model_state_dict_new


def load_state_dict(model, loaded_state_dict, no_head, excluded=[]):
    ...
    if len(excluded) > 0:
        ...
        loaded_state_dict = exclude_layers(loaded_state_dict, excluded)
