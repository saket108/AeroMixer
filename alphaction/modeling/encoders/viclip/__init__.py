...
def get_clip(name='viclip', weight_file=None):
    global clip_candidates
    m = clip_candidates[name]
    if m is None:
        if name == 'viclip':
            tokenizer = _Tokenizer()
            vclip = ViCLIP(tokenizer, pretrain=weight_file)
            # m = vclip
            m = (vclip, tokenizer)
        else:
...
def retrieve_text(frames, texts, name='viclip', weight_file=None, topk=5, device=torch.device('cuda')):
    clip, tokenizer = get_clip(name, weight_file)
    clip = clip.to(device)
