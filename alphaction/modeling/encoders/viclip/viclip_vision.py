...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.grid_size = [input_resolution // patch_size, input_resolution // patch_size]  # [16,16], or [14,14]
    ...
    def forward(self, x, interpolator=None, masking_prob=0.0, return_patch=False):
        ...
        pos_embed = self.positional_embedding.to(x.dtype)  # (257, D)
        if (interpolator is not None) and (pos_embed.shape[0] != ws[0] * ws[1] + 1):
            pos_embed = interpolator(pos_embed.unsqueeze(0), self.grid_size, ws, 1)  # (1, 18*?+1, D)
        x = x + pos_embed
        ...
        if hasattr(self, 'temporal_positional_embedding'):
            if x.size(1) == 1:
                ...
                pe_temporal = self.temporal_positional_embedding
                if (interpolator is not None) and (pe_temporal.size(1) != x.size(1)):
                    pe_temporal = interpolator(pe_temporal, (1, pe_temporal.size(1)), (1, x.size(1)), 0)
                x = x + pe_temporal
