...
class ViT(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        tubelet_size = cfg.ViT.TUBELET_SIZE  # 2
        patch_size = cfg.ViT.PATCH_SIZE  # 16
        in_chans = cfg.ViT.IN_CHANS  # 3
        embed_dim = cfg.ViT.EMBED_DIM  # 768
        pretrain_img_size = cfg.ViT.PRETRAIN_IMG_SIZE  # 224
        use_learnable_pos_emb = cfg.ViT.USE_LEARNABLE_POS_EMB  # false
        drop_rate = cfg.ViT.DROP_RATE  # 0
        attn_drop_rate = cfg.ViT.ATTN_DROP_RATE  # 0
        drop_path_rate = cfg.ViT.DROP_PATH_RATE  # 0.2
        depth = cfg.ViT.DEPTH  # 12
        num_heads = cfg.ViT.NUM_HEADS  # 12
        mlp_ratio = cfg.ViT.MLP_RATIO  # 4 
        qkv_bias = cfg.ViT.QKV_BIAS  # true
        qk_scale = cfg.ViT.QK_SCALE  # none
        init_values = cfg.ViT.INIT_VALUES  # 0
        use_checkpoint = cfg.ViT.USE_CHECKPOINT  # true
        ...
        self.patch_embed = PatchEmbed(img_size=pretrain_img_size, patch_size=patch_size, in_chans=in_chans,
                                      embed_dim=embed_dim, tubelet_size=self.tubelet_size)  # (16, 224, 224) --> (8, 224//16, 224//16) by Conv3D
        num_patches = self.patch_embed.num_patches  # 8x14x14 = 1568
        ...
        self.blocks = nn.ModuleList([
            ...
            for i in range(depth)])  # 12 blocks, drop_path rate linearly increases from 0 to 0.2
        self.norm = norm_layer(embed_dim)
        self.dim_embed = embed_dim


    def forward(self, x_list):

        x = x_list[0]  # slow video (b, 3, 16, 256, ?)
        # print(x.shape) b, 3, 16, 244, 244
        x = self.patch_embed(x)  # x.shape=[b 768 8 16 ?]  has a Conv3D
        ws = x.shape[2:]  # t=8,h=16,w=?
        num_frame = x.shape[2]
        assert num_frame % 2 == 0, "Only consider even case, check frames {}".format(num_frame)

        x = x.flatten(2).transpose(1, 2).contiguous()  # (b, 768, 8x16x?) --> b,thw,768
        B, _, C = x.shape

        pos_embed = self.pos_embed  # 1, =8*14*14, 768
        if self.pos_embed.shape[1] != ws[0] * ws[1] * ws[2]:
            # pos_embed=[1 8*14*14 384]->[1 8*16*29 384]
            pos_embed = pos_embed.reshape(ws[0], -1, C)  # (8, 196, 768)
