"""
IA Structure for Image Multimodal Models.
...
class InteractionBlock(nn.Module):
    """
    Interaction Block for 2D image features.
    Handles person-object interactions for image multimodal.
    """
    
    def __init__(self, dim_person, dim_other, dim_out, dim_inner, structure_config,
                 max_others=20,
                 temp_pos_len=-1,  # Not used in image mode, kept for compatibility
                 dropout=0.):
        ...
        # temp_pos_len is not used in image mode (kept for compatibility)
        self.temp_pos_len = -1  # Force to -1 for image mode

        bias = not structure_config.NO_BIAS
        init_std = structure_config.CONV_INIT_STD

        # Converted from Conv3d to Conv2d for image
        self.query = nn.Conv2d(dim_person, dim_inner, 1, bias)
        init_layer(self.query, init_std, bias)

        # Converted from Conv3d to Conv2d for image
        self.key = nn.Conv2d(dim_other, dim_inner, 1, bias)
        init_layer(self.key, init_std, bias)

        # Converted from Conv3d to Conv2d for image
        self.value = nn.Conv2d(dim_other, dim_inner, 1, bias)
        init_layer(self.value, init_std, bias)

        # Converted from Conv3d to Conv2d for image
        self.out = nn.Conv2d(dim_inner, dim_out, 1, bias)
        ...
        if dim_person != dim_out:
            # Converted from Conv3d to Conv2d for image
            self.shortcut = nn.Conv2d(dim_person, dim_out, 1, bias)
    ...
    def forward(self, person, others):
        """
        :param person: [n, channels, h, w] - 4D for image
        :param others: [n, num_other, channels, h, w] - 4D for image
        """
        device = person.device
        n, dim_person, h, w = person.size()  # 4D: (n, c, h, w)
        _, max_others, dim_others, h_others, w_others = others.size()  # 5D: (n, num, c, h, w)

        query_batch = person
        # Fuse batch and num dimensions for processing
        key = fuse_batch_num(others)  # [n*num_other, channels, h, w]

        query_batch = self.query(query_batch)
        # Reshape for attention: [n, num_other, dim_inner, h, w]
        key_batch = self.key(key).contiguous().view(n, self.max_others, self.dim_inner, h_others, w_others)
        value_batch = self.value(key).contiguous().view(n, self.max_others, self.dim_inner, h_others, w_others)

        # Flatten spatial dimensions for attention: [n, dim_inner, h*w]
        query_batch = query_batch.contiguous().view(n, self.dim_inner, -1).transpose(1, 2)  # [n, hw, dim_inner]
        key_batch = key_batch.contiguous().view(n, self.max_others, self.dim_inner, -1).transpose(2, 3)
        key_batch = key_batch.contiguous().view(n, self.max_others * h_others * w_others, -1).transpose(1, 2)  # [n, max_other*hw, dim_inner]

        qk = torch.bmm(query_batch, key_batch)  # n, hw, max_other * hw
        ...
        value_batch = value_batch.contiguous().view(n, self.max_others * h_others * w_others, -1)
        out = torch.bmm(weight, value_batch)  # n, hw, dim_inner

        out = out.contiguous().view(n, h * w, -1)
        out = out.transpose(1, 2)
        out = out.contiguous().view(n, self.dim_inner, h, w)

        if self.use_ln:
            if not hasattr(self, "layer_norm"):
                self.layer_norm = nn.LayerNorm([self.dim_inner, h, w], elementwise_affine=False).to(device)
...
class IAStructure(nn.Module):
    """
    IA Structure for Image Multimodal.
    Handles person-object interactions in images.
    """
    ...
    def __init__(self, dim_person, dim_mem, dim_out, structure_cfg):
        ...
        # For image mode, memory is not used (set to 1)
        self.mem_len = 1
        ...
        self.has_M = has_memory(structure_cfg)  # Not used in image mode

        # Converted from Conv3d to Conv2d for image
        self.person_dim_reduce = nn.Conv2d(dim_person, self.dim_inner, 1, bias)  # reduce person query
        init_layer(self.person_dim_reduce, conv_init_std, bias)
        self.reduce_dropout = nn.Dropout(structure_cfg.DROPOUT)

        if self.has_M:
            # Converted from Conv3d to Conv2d for image
            self.mem_dim_reduce = nn.Conv2d(dim_mem, self.dim_inner, 1, bias)
            init_layer(self.mem_dim_reduce, conv_init_std, bias)
        if self.has_P:
            # Converted from Conv3d to Conv2d for image
            self.person_key_dim_reduce = nn.Conv2d(dim_person, self.dim_inner, 1, bias)  # reduce person key
            init_layer(self.person_key_dim_reduce, conv_init_std, bias)
        if self.has_O:
            # Converted from Conv3d to Conv2d for image
            self.object_dim_reduce = nn.Conv2d(dim_person, self.dim_inner, 1, bias)
            init_layer(self.object_dim_reduce, conv_init_std, bias)

    def forward(self, person, person_boxes, obj_feature, object_boxes, mem_feature=None):
    ...
    def _reduce_dim(self, person, person_boxes, obj_feature, object_boxes, mem_feature):
        ...
        if self.has_M and mem_feature is not None:
    ...
    def _make_interaction_block(self, block_type, block_name, dim_person, dim_other, dim_out, dim_inner, structure_cfg):
        ...
        temp_pos_len = -1  # Not used in image mode
        if block_type == "P":
            ...
            # temporal_position not used in image mode
            temp_pos_len = -1
        else:
...
class DenseSerialIAStructure(SerialIAStructure):
    def __init__(self, dim_person, dim_mem, dim_out, structure_cfg):
        ...
        for idx in range(2, len(self.I_block_list)):
            # Converted to 4D for image: (idx, dim_inner, 1, 1) instead of 5D
            init_tensor = torch.full((idx, self.dim_inner, 1, 1), -5 * math.log(10), dtype=torch.float32)
            init_tensor[idx - 1, ...] = 0
...
def separate_roi_per_person(proposals, things, other_proposals, max_things):
    """
    :param things: [n2, c, h, w] - 4D for image
    :param proposals:
    :param max_things:
    :return [n, max_other, c, h, w] - 4D for image
    """
    res = []
    _, c, h, w = things.size()  # 4D for image
...
def separate_batch_per_person(proposals, things):
    ...
    :param things: [b, max_others, c, h, w] - 4D for image
    :return [n, max_others, c, h, w] - 4D for image
    """
    res = []
    b, max_others, c, h, w = things.size()  # 4D for image
    device = things.device
    for i, person_box in enumerate(proposals):
        person_num = len(person_box)
        tmp = torch.zeros((person_num, max_others, c, h, w), device=device)
        tmp[:] = things[i]
        res.append(tmp)
...
def fuse_batch_num(things):
    n, number, c, h, w = things.size()  # 4D for image
