...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.uniform_params = None
        if kwargs.get('sampling', False):
            a, b = kwargs.get('a', 0.0), kwargs.get('b', 1.0)
            assert a >=0 and a <=1 and b >=0 and b <=1 and a < b, "invalid a and b."
            self.uniform_params = [a, b]
        ...
        self.fuse_coeff = kwargs.get('fuse_coeff', -1)
        if 'fuse_coeff' in kwargs:
            kwargs.pop('fuse_coeff')
    ...
    def forward(self,
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        query_in = self.ln_q(query) if self.use_ln else query
        ...
        if self.uniform_params is not None:
            if self.training:
                # generate the same number of random factors as the the keys
                factors = torch.FloatTensor(1, key.size(0), 1).uniform_(self.uniform_params[0], self.uniform_params[1]).to(out.device)
            else:
                expectation = 0.5 * (self.uniform_params[0] + self.uniform_params[1])
